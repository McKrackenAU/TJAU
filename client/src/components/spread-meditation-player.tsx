...
import { Loader2, RefreshCw, Play, Pause, Music, Volume2 } from "lucide-react";
...
interface SpreadMeditationPlayerProps {
  cards: TarotCard[];
  spreadType: string;
  positions: string[];
}

export default function SpreadMeditationPlayer({ 
  cards, 
  spreadType,
  positions
}: SpreadMeditationPlayerProps) {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: [`/api/meditate-spread`, cards.map(c => c.id), spreadType],
    queryFn: async () => {
      try {
        const res = await apiRequest("POST", "/api/meditate-spread", {
        ...
        console.error("Error fetching spread meditation:", err);
        throw new Error(err instanceof Error ? err.message : "Failed to generate meditation");
      }
    },
    enabled: isRequested
  });
    ...
    return () => {
      ...
      if (oscillatorRef.current) {
        try {
          ...
          console.error("Error cleaning up oscillator:", err);
        }
      }
  ...
  const handleMusicTypeChange = (type: MeditationMusicType) => {
    ...
    if (musicType !== 'none') {
      ...
      if (youtubePlayerRef.current && (musicType === 'nature' || musicType === 'crystal')) {
        try {
  ...
  const handleYouTubeReady = (event: any) => {
    ...
    if (isPlaying && ['nature', 'crystal'].includes(musicType)) {
      try {
        ...
        console.error("Error with YouTube player on ready:", err);
      }
    } else {
      try {
        event.target.setVolume(0);
      } catch (err) {
        console.error("Error setting YouTube volume to 0:", err);
      }
    }
  };
  ...
  const handlePlay = () => {
    try {
      ...
      if (!audio && data?.audioUrl) {
        console.log("Creating new audio instance for spread meditation");
  ...
  if (!isRequested) {
    return (
      <Button
        ...
        onClick={() => {
          console.log("Requesting spread meditation");
          setIsRequested(true);
        }}
      >
        <Volume2 className="w-4 h-4 mr-2" />
        Get Guided Spread Meditation
      </Button>
    );
  }
  ...
  if (error) {
    console.error("Spread meditation error:", error);
    return (
      ...
      <CardContent className="pt-6">
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <h4 className="text-lg font-semibold">Spread Guided Meditation</h4>
            <Button 
              variant="ghost" 
              size="sm"
